...
VIEW_TIPS: dict[str, str] = {
    "PLAX": "补扫 PLAX：左侧卧位优先，从胸骨左缘第 3-4 肋间开始，先找到二尖瓣、主动脉瓣和左室长轴。",
...
def build_primary_care_guidance(study: StudyAnalysis, diagnosis_label: str = "") -> PrimaryCareGuidance:
    ...
    if not tips and study.quality_score < 0.72:
        tips.append("体位基本齐全，但图像质量仍可提升：优先减少心尖切短，调整增益，并保存连续心动周期。")

    safety_level, next_steps = triage_text(study, diagnosis_label, missing)
    present_text = ", ".join(sorted(present_focus)) if present_focus else "无"
    tips_text = "建议补充：" + "；".join(tip.rstrip("。") for tip in tips) + "。" if tips else "当前基础切面覆盖尚可。"
    compact = (
        f"基层/初学者辅助提示：FoCUS 五切面完整性为{completeness_level}，"
        f"已见 {len(present_focus)}/5 个基础切面（{present_text}），"
        f"图像质量为{quality_level}。{tips_text} "
...
def triage_text(study: StudyAnalysis, diagnosis_label: str, missing_views: list[str]) -> tuple[str, str]:
    abnormal = any(keyword in diagnosis_label for keyword in IMPORTANT_LABELS)
    uncertain = "证据不足" in diagnosis_label or study.quality_score < 0.45 or study.systole_count == 0 or study.diastole_count == 0
    severe_gap = len(missing_views) >= 3

    if abnormal and study.quality_score >= 0.45:
        return (
            "建议尽快上级复核",
            "若患者伴胸痛、晕厥、明显呼吸困难、低血压、发绀或急性心衰表现，应优先转诊或联系上级超声/心内科复核。",
        )
    ...
    return (
        "结合临床复核",
        "建议把本结果作为教学和初筛参考，结合病史、体征、心电图、BNP/肌钙蛋白等可获得资料，并由有资质医师复核。",
    )
