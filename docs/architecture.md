...
flowchart TD
    ...
    D --> D1["HSV 血流向量化"]
    D1 --> D2["连通域 / 喷流宽度 / 涡量代理 / 瓣膜定位评分"]
    C3 --> E["相位与体位聚合"]
    D2 --> E
    E --> O["轻量离线多智能体编排<br/>InputAgent / FeatureAgent / DiagnosisAgent"]
    O --> F["规则后备 + EchoNet-Dynamic EF 校准"]
    F --> G["Gemma4 4B 结构化 JSON 输出<br/>llama-cli / llama-server"]
    G --> R["ReportAgent / SafetyAuditAgent<br/>报告守卫 + 审计 JSON"]
    R --> H["中文教学参考病症判断"]
```
...
- PC V5 使用轻量离线多智能体编排：各 agent 只做本地摘要、决策复用、报告合同检查和安全审计，不额外联网，也不额外多次调用 Gemma4。
