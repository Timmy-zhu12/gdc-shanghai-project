...
本仓库保持为“源代码 + 文档 + 验证摘要 + 可复现脚本”的结构，不保存需要额外授权的医学原始数据或大模型权重。完整数据集来源、用途和再分发边界见 [../DATASETS.md](../DATASETS.md)。
...
CardioConsult 的 Gemma4 主链路接收结构化超声证据，而不是原始像素或完整 DICOM。B-mode、Color Doppler 和动图分支先在本机提取低维代理特征，Gemma4 再基于这些特征进行函数调用、层级诊断组织和报告生成。这样做可以降低隐私暴露面，也便于审计模型到底依据了哪些证据。
