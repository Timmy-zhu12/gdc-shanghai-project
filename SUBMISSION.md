...
| 5 分钟内演示视频 | 仓库内视频：[submission/demo_video/demo.mp4](submission/demo_video/demo.mp4)；录制脚本见 [submission/demo_video/CardioConsult_5min_demo_script_CN.md](submission/demo_video/CardioConsult_5min_demo_script_CN.md) 与 DOCX 版 | 已准备 |
...
| 技术能力 | 25% | B-mode GLDM/纹理代理、SRAD/CLAHE 预处理、Color Doppler HSV/向量代理、Doppler 瓣膜定位评分、动图/DICOM 支持、EchoNet-Dynamic EF 校准、本地 Gemma4 4B、原生函数调用白名单 |
...
- 函数调用可复现：`tools/function_calling_smoke.py` 可在无 GGUF、无网络条件下验证 `summarize_ultrasound_features`、`run_rule_diagnosis` 和 `safety_boundary_check` 三个内部工具，以及非法工具调用拒绝逻辑。
...
- 演示视频已放入 `submission/demo_video/demo.mp4`；提交表单可填写仓库链接或按现场要求另传视频平台链接。
